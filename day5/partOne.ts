...
const partOne = () => {
  ...
  //uma funcao com 3 entradas: Ncrates, sourcePos, targetPos
  //localizar a stack baseado no sourcePos
  //pegar crate (Ncrates vezes -> for)
  //e pra cada crate que pegar, colocar na stack targetPos
  //devolver todas stacks organizadas
  ...
  const maguilaMudancas = (
    ...
    for (let i = 1; i <= Ncrates; i++) {
      crateToMove = stacks[realSourcePos].shift();
      stacks[realTargetPos].unshift(String(crateToMove));
    }
    return stacks;
  };
